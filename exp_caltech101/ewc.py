...
def residual_block_first(x, w1, w2,w3, b1, b2, strides1,strides2, pad="SAME"):
    ...
    # Shortcut connection
    if in_channel == w2.get_shape().as_list()[-1]:  # 输入和输出通道数相同的情况
        if strides1[1] == 1:
...
minibatch_size = 128
Lamb_set = [12,15,17,19,21] # hyper-parameter1 loss
epoches = 3000
learning_lr = 0.001
...
for Lamb in Lamb_set:
    ...
    for task in range(num_tasks_to_run):
        ...
        if task == 0:
            ...
            model.restore(sess)
            cost_new = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=y_[task], logits=task_output[task]))
        ...
        for epoch in range(1,num_epochs_per_task+1):
            ...
            for i in range(int(len(task_permutation[task][0]) / minibatch_size) + 1):
                ...
                train_batch_img=_random_flip_leftright(train_batch_x)  #数据增强
        ...
        shuffle__img,shuffle__lab=shuffle(len(task_permutation[task][0]),task_permutation[task][0],task_permutation[task][1])
        train_=read_images(shuffle__img)
        if task == 0:
            Omega_v = compute_omega(sess,train_, batch_size=100,param_im=param_importance)
        else:
            for l in range(len(Omega_v)):
                Omega_v[l] = Omega_v[l] + compute_omega(sess,train_, batch_size=100, param_im=param_importance)[l]
