...
def Cal_importance(varlist,output):
    ...
    for v in range(len(varlist)):
        im = tf.gradients(tf.nn.l2_loss(probs),varlist[v])
...
minibatch_size = 128
Lamb_set = [15] # hyper-parameter1 loss
epoches = 3000
learning_lr = 0.001
...
for Lamb in Lamb_set:
    ...
    for task in range(num_tasks_to_run):
        ...
        if task == 0:
            Omega_v = compute_omega(sess,train_, batch_size=50,param_im=param_importance)
        else:
            for l in range(len(Omega_v)):
                Omega_v[l] = Omega_v[l] + compute_omega(sess,train_, batch_size=50, param_im=param_importance)[l]
