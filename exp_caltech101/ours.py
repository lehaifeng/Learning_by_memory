...
minibatch_size = 128
Lamb_set = [25] # hyper-parameter1 loss
epoches = 3000
learning_lr = 0.001
...
for Lamb in Lamb_set:
    ...
    for task in range(num_tasks_to_run):
        ...
        if task == 0:
            Omega_v = compute_omega(sess, train_, batch_size=50,param_im=param_importance,sharpen=False,beta=Beta)
        else:
            for l in range(len(Omega_v)):
                Omega_v[l] = np.maximum(Omega_v[l],compute_omega(sess,train_, batch_size=50, param_im=param_importance,sharpen=False,beta=Beta)[l])
