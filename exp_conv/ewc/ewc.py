...
x1, tx1, y1, ty1 = svhn_for_library()
x2, tx2, y2, ty2 = notMNIST_for_library()
...
for Lamb in Lamb_set:
	...
	for task in range(num_tasks_to_run):
		...
		print("task", task, num_epochs_per_task, learning_r)
		for epoch in range(num_epochs_per_task):
			if epoch % 5 == 0:
				print("\t Epoch ", epoch)
				acc = 0
				step = 0
				for images, labels in yield_mb(tasks_test[task], tasks_test_label[task], batchsize=256,
