...
def train_task(model, num_iter, disp_freq, tasks_train, tasks_train_label, tasks_test, tasks_test_label, x, y_,keep_prob,lams):    
    ...
    # lams[l] sets weight on old task(s)
        ...
        for iter in range(num_iter):
            ...
            if iter % disp_freq == 0:
                for task in range(len(tasks_test)):
                    feed_dict={x: tasks_test[task], y_: label_to_one_hot(tasks_test_label[task], 10), keep_prob:1}
...
print(x0.shape)
print(y0.shape)
acc1=train_task(model,100,5, [x0], [y0], [tx0], [ty0],x, y_,keep_prob,lams=[0])
print("1th task acc:",acc1[0][-1])
# save current optimal weights
model.star()

acc2=train_task(model,100,5,[x1], [y1], [tx1], [ty1],x, y_,keep_prob,lams=[0])
print("2th task acc:",acc2[0][-1])
model.star()

acc3=train_task(model,100,5,[x2], [y2], [tx2], [ty2],x, y_,keep_prob,lams=[0])
print("3th task acc:",acc3[0][-1])
model.star()

acc4=train_task(model,100,5 ,[x3], [y3], [tx3], [ty3], x, y_,keep_prob,lams=[0])
print("4th task acc:",acc4[0][-1])
model.star()


acc5=train_task(model,100, 5,[x4], [y4], [tx0, tx1, tx2, tx3, tx4], [ty0, ty1, ty2, ty3, ty4], x, y_, keep_prob,lams=[0])
