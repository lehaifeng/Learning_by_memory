...
class Model:
    ...
    def compute_w1(self):
        ...
        for v in range(len(self.var_list)):
            im=tf.gradients(-tf.reduce_sum(self.probs1 * tf.log(self.probs1)),self.var_list[v])
            importance.append(tf.reduce_mean(tf.maximum(
                        im*self.var_list[v]+1/2*tf.multiply(tf.square(self.var_list[v]),tf.square(im)),0),0))
        return importance
    def compute_w2(self):
        importance=[]
        for v in range(len(self.var_list)):
            im=tf.gradients(-tf.reduce_sum(self.probs2 * tf.log(self.probs2)),self.var_list[v])
            importance.append(tf.reduce_mean(tf.maximum(
                        im*self.var_list[v]+1/2*tf.multiply(tf.square(self.var_list[v]),tf.square(im)),0),0))
        return importance
    def compute_w3(self):
        importance=[]
        for v in range(len(self.var_list)):
            im=tf.gradients(-tf.reduce_sum(self.probs3 * tf.log(self.probs3)),self.var_list[v])
            importance.append(tf.reduce_mean(tf.maximum(
                        im*self.var_list[v]+1/2*tf.multiply(tf.square(self.var_list[v]),tf.square(im)),0),0))
        return importance
    def compute_w4(self):
        importance=[]
        for v in range(len(self.var_list)):
            im=tf.gradients(-tf.reduce_sum(self.probs4 * tf.log(self.probs4)),self.var_list[v])
            importance.append(tf.reduce_mean(tf.maximum(
                        im*self.var_list[v]+1/2*tf.multiply(tf.square(self.var_list[v]),tf.square(im)),0),0))
        return importance
    def compute_w5(self):
        importance=[]
        for v in range(len(self.var_list)):
            im=tf.gradients(-tf.reduce_sum(self.probs5 * tf.log(self.probs5)),self.var_list[v])
            importance.append(tf.reduce_mean(tf.maximum(
                        im*self.var_list[v]+1/2*tf.multiply(tf.square(self.var_list[v]),tf.square(im)),0),0))
        return importance
    ...
    def set_vanilla_loss1(self):
        self.train_step1 = tf.train.GradientDescentOptimizer(0.01).minimize(self.cross_entropy1)
    def set_vanilla_loss2(self):
        self.train_step2 = tf.train.GradientDescentOptimizer(0.01).minimize(self.cross_entropy2)
    def set_vanilla_loss3(self):
        self.train_step3 = tf.train.GradientDescentOptimizer(0.01).minimize(self.cross_entropy3)
    def set_vanilla_loss4(self):
        self.train_step4 = tf.train.GradientDescentOptimizer(0.01).minimize(self.cross_entropy4)
    def set_vanilla_loss5(self):
        self.train_step5 = tf.train.GradientDescentOptimizer(0.01).minimize(self.cross_entropy5)

    def update_Hessian_loss1(self, lam):
        if not hasattr(self, "Hessian_loss1"):
            self.Hessian_loss1 = self.cross_entropy1
        for v in range(len(self.var_list)):
            self.Hessian_loss1 += (lam / 2) * tf.reduce_sum(tf.multiply(self.last_omega[v], tf.square(self.var_list[v] - self.star_vars[v])))
        self.train_step1 = tf.train.GradientDescentOptimizer(0.01).minimize(self.Hessian_loss1)
    def update_Hessian_loss2(self, lam):
        if not hasattr(self, "Hessian_loss2"):
            self.Hessian_loss2 = self.cross_entropy2
        for v in range(len(self.var_list)):
            self.Hessian_loss2 += (lam / 2) * tf.reduce_sum(tf.multiply(self.last_omega[v], tf.square(self.var_list[v] - self.star_vars[v])))
        self.train_step2 = tf.train.GradientDescentOptimizer(0.01).minimize(self.Hessian_loss2)
    def update_Hessian_loss3(self, lam):
        if not hasattr(self, "Hessian_loss3"):
            self.Hessian_loss3 = self.cross_entropy3
        for v in range(len(self.var_list)):
            self.Hessian_loss3 += (lam / 2) * tf.reduce_sum(tf.multiply(self.last_omega[v], tf.square(self.var_list[v] - self.star_vars[v])))
        self.train_step3 = tf.train.GradientDescentOptimizer(0.01).minimize(self.Hessian_loss3)
    def update_Hessian_loss4(self, lam):
        if not hasattr(self, "Hessian_loss4"):
            self.Hessian_loss4 = self.cross_entropy4
        for v in range(len(self.var_list)):
            self.Hessian_loss4 += (lam / 2) * tf.reduce_sum(tf.multiply(self.last_omega[v], tf.square(self.var_list[v] - self.star_vars[v])))
        self.train_step4 = tf.train.GradientDescentOptimizer(0.01).minimize(self.Hessian_loss4)
    def update_Hessian_loss5(self, lam):
        if not hasattr(self, "Hessian_loss5"):
            self.Hessian_loss5 = self.cross_entropy5
        for v in range(len(self.var_list)):
            self.Hessian_loss5 += (lam / 2) * tf.reduce_sum(tf.multiply(self.last_omega[v], tf.square(self.var_list[v] - self.star_vars[v])))
        self.train_step5 = tf.train.GradientDescentOptimizer(0.01).minimize(self.Hessian_loss5)
    ...
    def compute_omega1(self, sess, imgset, batch_size,keep_prob):
        ...
        O_im=[]
        for i in self.Omega_M:
            temp= np.exp(3*i) / np.sum(np.exp(3*i))
            O_im.append(temp)
        self.Omega_M=np.array(O_im)
        return self.Omega_M
